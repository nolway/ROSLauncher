        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour utiliser le lanceur, vous devez être connecté à au moins un compte. Vous devrez vous reconnecter après.<br><br>Êtes-vous sûr de vouloir vous déconnecter ?',
                    'Je suis sûr',
                    'Annuler'
                )
                setOverlayHandler(() => {
...
function resolveDropinModsForUI(){
    ...
    for(dropin of CACHE_DROPIN_MODS){
        dropinMods += `<div id="${dropin.fullName}" class="settingsBaseMod settingsDropinMod" ${!dropin.disabled ? 'activé' : ''}>
                    <div class="settingsModContent">
                        <div class="settingsModMainWrapper">
                            <div class="settingsModStatus"></div>
                            <div class="settingsModDetails">
                        ...
                        <label class="toggleSwitch">
                            <input type="checkbox" formod="${dropin.fullName}" dropin ${!dropin.disabled ? 'vérifié' : ''}>
                            <span class="toggleSwitchSlider"></span>
                        </label>
                    </div>
                </div>`
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Impossible de basculer<br>Un ou plusieurs mods Drop-in',
                            err.message,
                            'D\'accord'
                        )
                        setOverlayHandler(null)
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = 'Version expérimental'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/nolway/ROSLauncher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'Impossible de charger les notes de publication.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nouvelle version ${isPrerelease(data.version) ? 'expérimental' : 'stable'} disponible`
